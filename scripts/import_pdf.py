...
def infer_tags(text):
    tags = []
    for keyword, tag in [('Sentinel', 'Microsoft Sentinel'), ('Defender', 'Microsoft Defender'),
                         ('KQL', 'KQL'), ('query', 'KQL'), ('Cloud Apps', 'Cloud Apps'),
                         ('Azure AD', 'Microsoft Entra ID'), ('Purview', 'Microsoft Purview')]:
...
def parse_question(item):
    ...
    typ = 'hotspot' if 'HOTSPOT' in stem else 'matching' if 'DRAG DROP' in stem else 'multiple' if len(answer)>1 else 'single'
    reasons = []
    ...
    case_en = ''
    if 'Introductory Info' in stem:
        # Case materials occur before the actual question on a later page.
        split = re.split(r'\bQuestion\s*[-–:]\s*', stem, maxsplit=1)
        if len(split)==2:
            case_en, stem = split
        else:
            reasons.append('案例题干分界需核对')
    tags, domain = infer_tags(stem)
    return dict(id=item['id'], topic=item['topic'], number=item['number'], pages=item['pages'],
                type=typ, en=stem, zh='', options=opts, answer=answer, slots=[],
                explanation_en='', explanation_zh='', reference=re.findall(r'https?://\S+', after),
                case_en=case_en, case_zh='', assets=[], answer_assets=[], tags=tags,
                domain=domain, domain_status='inferred', status='review' if reasons else 'ready',
                issues=reasons, translation_status='pending', votes=votes)
...
def main():
    ...
    for i,p in enumerate(pdf.pages):
        ...
        if match:
            current=dict(id=f't{match[2]}-q{match[1]}', topic=int(match[2]), number=int(match[1]), pages=[], chunks=[])
            items.append(current)
            text=text[match.end():]
        if current:
            current['pages'].append(i+1); current['chunks'].append(text)
    questions=[parse_question(x) for x in items]
    cases={}
    for q in questions:
        if q['case_en']: cases[q['topic']]=q['case_en']
    for q in questions:
        q['case_en']=cases.get(q['topic'],'')
    if args.assets:
    ...
    bank=dict(version=digest[:12], source=Path(args.pdf).name, page_count=len(pdf.pages), questions=questions)
    (ROOT/'data').mkdir(exist_ok=True)
    (ROOT/'data/question-bank.json').write_text(json.dumps(bank,ensure_ascii=False,indent=2))
    report=dict(source_pages=len(pdf.pages), total=len(questions), unique_ids=len({q['id'] for q in questions}),
                topics=dict(Counter(q['topic'] for q in questions)), types=dict(Counter(q['type'] for q in questions)),
                statuses=dict(Counter(q['status'] for q in questions)),
                translated=sum(bool(q['zh']) for q in questions),
                review=[dict(id=q['id'],pages=q['pages'],issues=q['issues']) for q in questions if q['status']!='ready'])
    ...
    with pdfplumber.open(path) as pdf:
        for q in questions:
            ...
            if q['number']==1 and q['case_en']:
                ...
                pass
            if q['number']%30==0: print('assets',q['id'],flush=True)
    for q in questions:
        first=next((x for x in questions if x['topic']==q['topic'] and x['number']==1 and x['case_en']),None)
