...
def main():
    ...
    with httpx.Client(base_url=a.url,timeout=120) as c:
        if a.action=='submit':
            with open(a.target,'rb') as f: r=c.post('/api/ocr',files={'file':(Path(a.target).name,f,'application/pdf')},data={'dpi':'200'})
        elif a.action in ('retry','cancel'): r=c.post(f'/api/jobs/{a.target}/{a.action}')
        elif a.action=='download':
            status=c.get(f'/api/jobs/{a.target}'); status.raise_for_status()
            if status.json()['status']!='completed': raise SystemExit('任务尚未完成：'+status.text)
            r=c.get(f'/api/jobs/{a.target}/result',params={'format':'json'}); r.raise_for_status()
            Path(a.output).write_text(json.dumps(r.json(),ensure_ascii=False,indent=2))
            print(a.output); return
        else: r=c.get(f'/api/jobs/{a.target}')
        r.raise_for_status(); print(json.dumps(r.json(),ensure_ascii=False,indent=2))
